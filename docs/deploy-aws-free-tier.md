...
# 4) Create root .env (see the Free-Tier block below), then start infra
#    The free-tier overlay owns all port bindings (127.0.0.1 only).
#    It also sets mem_limit, restart policy, and tuned commands.
...
| File                                                                                                      | Change                                                                                                                                                                                                                                                                                                                                          |
| --------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| [`docker-compose.yml`](../docker-compose.yml)                                                             | **Redesigned** — removed all `ports` entries. Base file now defines only image, environment, and volumes. This is the fix for the port-duplication bug: Docker Compose appends lists across overlays; owning ports exclusively in each overlay makes duplication structurally impossible. Also removed the obsolete `version: '3.8'` attribute. |
| [`docker-compose.dev.yml`](../docker-compose.dev.yml)                                                     | **[NEW]** Local development overlay — adds `0.0.0.0`-bound ports for Postgres (5432), Redis (6379), and MinIO (9000/9001). Used by `mprocs` / `npm run dev`.                                                                                                                                                                                    |
| [`docker-compose.freetier.yml`](../docker-compose.freetier.yml)                                           | **Fixed port-duplication bug** — now the sole owner of all port bindings (127.0.0.1 only). `restart: unless-stopped`, `mem_limit`, tuned `command` overrides, and Redis `allkeys-lru` policy. No ports in the base file means nothing to merge/append.                                                                                          |
| [`mprocs.yaml`](../mprocs.yaml)                                                                           | Updated `docker` proc to use `docker compose -f docker-compose.yml -f docker-compose.dev.yml up` so local dev still gets all ports bound.                                                                                                                                                                                                       |
